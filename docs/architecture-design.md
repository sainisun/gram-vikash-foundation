...
| Financial database | Donors, donations, expenses, programs, audit records and immutable source values | 1 | Managed PostgreSQL; all totals are derived from this source of truth [1] |
...
| Authentication and authorization | Admin sessions, role checks, donor receipt identity, community member verification | 1/2 | NextAuth.js or Supabase Auth with application-level authorization checks |
...
1. A donor selects an amount and optional program earmark on the public donation page. Amounts are converted to **paise** before any server request and validated as positive integers.
2. The server creates a pending donation intent and a Razorpay order using server-held credentials. The application stores the internal donation ID and Razorpay order ID, but never receives card, UPI, or bank credentials.
...
1. A community member signs in and requests a vote issue. The server checks that the issue is active, the member account is verified, the member is at least 18 under the project’s verification policy, and the account is not suspended.
...
6. The client refreshes results through polling in the initial implementation or a future push adapter. The result endpoint must filter by issue and current status and must not expose private identity fields.
...
SELECT
  ...
  (SELECT COUNT(DISTINCT donor_id) FROM donations WHERE status = 'success') AS donor_count;
```
