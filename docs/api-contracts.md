...
| `donor-session` | Short-lived donation/status token or authenticated donor session; may access only the donor’s own receipt/status |
| `community-member` | Authenticated community member; may browse/post/comment/report subject to account status |
| `community-member-verified` | Authenticated member with server-confirmed verification and voting eligibility |
...
**Auth:** `public`  
**Purpose:** Validate donor intent and create a Razorpay order plus a pending donation record.
...
// 201 Response
{
  ...
  donor_session_token: string;
}
```

Reject unsupported amounts, inactive program IDs, malformed contact data, or contradictory anonymity/display-name fields. Apply per-IP and per-device rate limits; never log the full donor body or payment credentials.
...
**Auth:** `donor-session` or `admin`  
**Purpose:** Return a donor’s own or an admin’s donation state.
...
Return `404` rather than revealing whether another donor’s ID exists. Do not return provider secrets or raw payment payloads.

### `GET /api/donations/:donation_id/receipt`

**Auth:** `donor-session` or `admin`  
**Purpose:** Return a short-lived signed download URL or stream the donor’s own successful receipt.
...
Return only donors who explicitly opted in to public display; otherwise use `Anonymous` or omit the entry according to the published donor-wall policy.
...
```ts
{
  donor: { name?: string; email?: string; phone?: string; is_anonymous: boolean };
  amount_paise: number;
  program_id?: string | null;
...
**Response:** `201 { donation: DonationAdminView; audit_log_id: string }`. Offline rows appear in the same public ledger after validation, but are entered with `entered_by_admin_id` and must never be presented as Razorpay payments.
...
**Auth:** `admin` or `community-member`  
**Request:** `{ purpose: 'expense_receipt'|'program_photo'|'community_media'; filename: string; content_type: string; size_bytes: number }`  
**Response:** `{ upload_url: string; object_key: string; expires_at: string }`. Validate type/size and place community media in quarantine. Never allow arbitrary bucket paths.

## 5. Community members, posts, comments, and chat

### `POST /api/community/members` and `GET /api/community/me`

**Auth:** `public` for registration; `community-member` for profile  
**Registration request:** `{ name: string; phone: string; email?: string; dob: string; village_ward: string; profile_photo_url?: string }`  
**Registration response:** `{ member_id: string; verification_status: 'unverified'; session_token: string }`. Registration is rate limited and phone verification is required before an account can post. Do not accept self-declared age as sufficient for voting.

`GET /api/community/me` returns the member’s own profile, status, and voting eligibility explanation, never another member’s PII.

### `GET /api/community/posts`

**Auth:** `public` or `community-member`  
...
**Auth:** `public` for published comments; `community-member` to create  
**Request:** `{ text: string }`  
...
**Auth:** `community-member-verified`  
...
### `PATCH /api/admin/community-members/:member_id/verification`

**Auth:** `admin`  
**Request:** `{ status: 'verified'|'rejected'|'suspended'; notes?: string }`  
**Response:** `{ member_id: string; verification_status: string; verified_at: string | null; audit_log_id: string }`. Manual verification is the recommended MVP approach; do not implement Aadhaar/e-KYC without a separate approved privacy and legal design. [1]
